...
else
{
	...
	{		
		...
		if($statuscode==STATUS_OFFLINE)
			$status = '<img class="serverlist_statusimg" id="status_'.$num.'" src="img/status/rolling.gif" height="25" title="This server is offline" alt="offline" />';
			...
			$status = '<img class="serverlist_statusimg" id="status_'.$num.'" src="img/status/online.gif" height="25" title="This server is online" alt="online" />';
		...
		$content .= <<<EOF
			<div id="row_$num" class="serverlist_row">
				...
				<div class="serverlist_actionfield">
					<img  src="img/srch.png"       height="25" title="View this server in more detail"  alt="view"   onClick="javascript: window.location.href = '?server=$servername&amp;action=server_control';" />
					<img  src="img/copy.png"   height="25" title="Copy this server configuration"   alt="copy"   onClick="javascript: copyServer($num, '$servername');" />
					<img  src="img/garbage.png"        height="25" title="Delete this server configuration" alt="delete" onClick="javascript: if(!$deleteArg) deleteServer($num, '$servername'); else alert('Cannot delete running servers!');" />
				</div>
			</div>
EOF;
